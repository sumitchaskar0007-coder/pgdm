...
export default function EnquiryModal({ open, onClose }) {
	...
	async function onSubmit(data) {
		...
		if (res.ok) {
			...
			alert(error?.message || 'Could not submit. Please try again.')
		}
	}
		...
		<div aria-modal="true" role="dialog" className="fixed inset-0 z-50 flex items-center justify-center">
			...
			<div className="relative bg-white rounded-2xl max-w-xl w-full p-6 mx-4 shadow-xl">
				...
				<form onSubmit={handleSubmit(onSubmit)} className="space-y-3 mt-4">
					<div>
						<input aria-invalid={!!errors.name} {...register('name',{ required: 'Name required'})} placeholder="Full name" className="w-full p-3 border rounded-lg border-gray-300 focus:border-primary focus:ring-primary" />
						{errors.name && <p className="text-sm text-red-600 mt-1">{errors.name.message}</p>}
					</div>
					<div>
						<input aria-invalid={!!errors.email} {...register('email',{ required: 'Email required', pattern: { value: /^\S+@\S+$/i, message:'Invalid email'} })} placeholder="Email" className="w-full p-3 border rounded-lg border-gray-300 focus:border-primary focus:ring-primary" />
						{errors.email && <p className="text-sm text-red-600 mt-1">{errors.email.message}</p>}
					</div>
					<div>
						<input aria-invalid={!!errors.phone} {...register('phone',{ required: 'Phone required', pattern: { value: /^[0-9]{10,15}$/, message: 'Enter 10–15 digits'} })} placeholder="Phone" className="w-full p-3 border rounded-lg border-gray-300 focus:border-primary focus:ring-primary" />
					...
					<label className="flex items-start gap-2 text-sm">
						<input {...register('consent',{ required: true })} type="checkbox" className="mt-1" />
						<span>I consent to be contacted about admission & programmes and agree to the <a href="/privacy" className="underline">privacy policy</a>.</span>
					</label>
					<div className="flex justify-end gap-2 pt-2">
						<button type="button" onClick={onClose} className="px-4 py-2 rounded-lg border">Cancel</button>
						<button disabled={isSubmitting} type="submit" className="px-4 py-2 bg-primary text-white rounded-lg">{isSubmitting ? 'Submitting...' : 'Submit'}</button>
					</div>
				</form>
			</div>
		</div>
