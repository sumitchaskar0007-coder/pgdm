...
export default function Videos() {
  ...
  return (
    <section className="min-h-screen bg-gray-100 py-10">
          ...
          {videos.map((video) => (
            <button key={video._id} type="button" onClick={() => setSelected(video)} className="text-left bg-white rounded-xl shadow overflow-hidden hover:-translate-y-1 transition">
              <div className="relative aspect-video bg-blue-950">
                ...
                <span className="absolute inset-0 flex items-center justify-center"><span className="w-16 h-16 rounded-full bg-white/90 text-blue-700 flex items-center justify-center text-2xl pl-1">▶</span></span>
              </div>
      ...
      {selected && <div className="fixed inset-0 z-[60] bg-black/90 p-4 flex items-center justify-center" onClick={() => setSelected(null)}><div className="w-full max-w-5xl" onClick={(e) => e.stopPropagation()}><button className="block ml-auto text-white text-3xl mb-2" onClick={() => setSelected(null)} aria-label="Close">×</button><VideoPlayer video={selected} className="w-full aspect-video bg-black" /><h2 className="text-white text-xl mt-3">{selected.title}</h2><p className="text-gray-300">{selected.description}</p></div></div>}
    </section>
  );
}
